    ...
    try {
      ...
      if (!error && data && data.length > 0) {
        return data.map(row => ({
          ...
          sector: row.sector,
          image: row.image_url || 'https://images.unsplash.com/photo-1532187863486-abf9dbad1b69?auto=format&fit=crop&w=800&q=80',
          title: row.title,
...
export const saveCorporateNeedSubmission = async (needData) => {
  const entry = {
    ...
    image: needData.image || 'https://images.unsplash.com/photo-1532187863486-abf9dbad1b69?auto=format&fit=crop&w=800&q=80',
